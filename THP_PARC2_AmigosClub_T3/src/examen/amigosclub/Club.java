...
public class Club {
	...
	public void listarSociosConAmigosEdadSimilar() {
		final int tolerancia = 1;
		if (socios.size() > 0) {
			System.out.println("Socios con amigos de edad similar (+-1 año):");
			for (Persona socio : this.socios) {
				int cantidad = socio.cantidadMismaEdad(tolerancia);
				System.out.println(socio.getNombre() + " - " + cantidad);
			}
		} else {
